...
export default function ModeratorDashboardPage() {
  ...
  const reportedComments = db.comments.filter(c => c.status === 'REPORTED');
  
  const recentLogs = useMemo(() => {
    return db.auditLogs.filter(log => log.targetType === 'COMMENT').slice(0, 5);
  }, [db.auditLogs]);
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        <div className="bg-surface-container-low border border-outline-variant rounded-lg p-6 flex flex-col gap-2 relative overflow-hidden group hover:border-outline transition-colors">
          <div className="flex justify-between items-start mb-2">
            <span className="font-label-sm text-label-sm text-on-surface-variant">TOTAL COMMENTS</span>
            <span className="material-symbols-outlined text-secondary opacity-70">chat_bubble</span>
          </div>
          <span className="font-display-lg text-display-lg text-on-background">{totalComments}</span>
        ...
        <div className="bg-surface-container border border-outline-variant rounded-lg p-6 flex flex-col gap-2 relative overflow-hidden group hover:border-outline transition-colors">
          ...
          <span className="font-display-lg text-display-lg text-on-background">{pendingComments.length}</span>
          <div className="flex items-center gap-1 text-label-sm font-label-sm mt-1">
            {pendingComments.length > 0 ? (
              ...
              <span className="text-on-surface-variant">Queue is clear</span>
            )}
          </div>
        ...
        <div className="bg-surface-container-low border border-outline-variant rounded-lg p-6 flex flex-col gap-2 relative overflow-hidden group hover:border-outline transition-colors">
          ...
          <span className="font-display-lg text-display-lg text-error">{reportedComments.length}</span>
          <div className="flex items-center gap-1 text-label-sm font-label-sm mt-1">
        ...
        <section className="lg:col-span-2 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-4">
            {recentLogs.length > 0 ? recentLogs.map(log => (
              <div key={log.id} className="flex gap-4 items-start p-4 bg-surface-container-lowest border border-outline-variant rounded-lg">
                <div className="mt-1 bg-surface-container rounded-full p-2 text-on-surface-variant">
                  <span className="material-symbols-outlined text-[20px]">
                    {log.action.includes('APPROVED') ? 'check_circle' : log.action.includes('REJECT') ? 'block' : 'info'}
                  </span>
                </div>
                <div className="flex-1">
                  <div className="flex justify-between items-baseline mb-1">
                    <span className="font-label-sm text-label-sm text-on-surface-variant">{log.action.replace(/_/g, ' ')}</span>
                    <span className="font-label-sm text-label-sm text-on-surface-variant opacity-70">
                      {new Date(log.timestamp).toLocaleDateString()}
                    </span>
                  </div>
                  <p className="font-body-md text-body-md text-on-background mb-2">{log.description}</p>
                </div>
              </div>
            )) : (
              <p className="font-body-md text-on-surface-variant italic text-center py-8">No recent moderation activity.</p>
            )}
          </div>
        </section>
        ...
        <aside className="flex flex-col gap-6">
          <div className="bg-surface-container-low border border-outline-variant rounded-lg p-6">
            ...
            <ul className="flex flex-col gap-4">
                ...
                <li key={comment.id} className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-surface-variant flex items-center justify-center text-on-surface-variant shrink-0">
                    <span className="font-label-sm text-label-sm">
                      {db.users.find(u => u.id === comment.userId)?.name.charAt(0) || 'U'}
                    </span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-body-md text-sm text-on-background truncate">{comment.content}</p>
                    <p className="font-label-sm text-xs text-on-surface-variant truncate">
                      Article: &quot;{db.articles.find(a => a.id === comment.articleId)?.title || 'Unknown'}&quot;
                    </p>
                  </div>
              ...
              {pendingComments.length === 0 && (
                <li className="text-on-surface-variant font-body-md italic py-2">Queue is empty</li>
              )}
            </ul>
